...
class Cleaning
{
    ...
    public function __construct(
        ...
        //Hook cron
        add_action('ad_integration_cleaning_duplicate_users', [$this, 'removeDuplicateUsers']);
        add_action('ad_integration_cleaning_orphan_meta', [$this, 'removeOphanUserMeta']);
        add_action('ad_integration_cleaning_capabilities', [$this, 'removeEmptyCapabilities']);
    }
    ...
    public function removeEmptyCapabilities()
    {
        try {
            ...
            $this->db->query('DELETE FROM ' . $this->db->usermeta . " WHERE meta_key LIKE '%" . $this->db->base_prefix . "_capabilities%' AND meta_value = 'a:0:{}' ");
            wp_cache_flush();
        } catch (\Exception $e) {
